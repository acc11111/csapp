    ...
    for (i = 0; i < 32; i += 8) {     // 枚举每八行
        for (j = 0; j < 32; j += 8) { // 枚举每八列
            ...
            for (int cnt = 0; cnt < 8;
                ...
                B[j][i] = temp1; // 第一次 这八个都会
                                 // miss,后面就会命中，当然对角线有些例外
                B[j + 1][i] = temp2;
                B[j + 2][i] = temp3;
                B[j + 3][i] = temp4;
                B[j + 4][i] = temp5;
...
void solve_64(int M, int N, int A[N][M], int B[M][N]) {
    ...
    for (i = 0; i < N; i += 8) {     // 枚举每八行
        for (j = 0; j < M; j += 8) { // 枚举每八列
            ...
            // 这是 左上，且将左下的块移动到
            // B中的右上，这样是为了更高效地利用cache
            for (cnt = 0; cnt < 4; ++cnt, ++i) { // 枚举0~8中的每一行，一行八列
                temp1 = A
                    [i]
                    [j]; // 这样我们就一次取出了8个元素，我们的A的miss就只有一次了
                         // 原始4*4 则是两次
                temp2 = A[i][j + 1];
                temp3 = A[i][j + 2];
                temp4 = A[i][j + 3]; // 左上
            ...
            for (cnt = 0; cnt < 4; ++j, ++cnt) { // 枚举0~8中的每一行，一行八列
                ...
                temp5 =
                    B[j][i + 4]; // 拿到我们之前赋给B右上的，也就是A右上的元素
                temp6 = B[j][i + 5];
                temp7 = B[j][i + 6];
                temp8 = B[j][i + 7];
                ...
                // 这一步，也不会带来更多的miss因为 i + 4 <= j + 4 <= i + 7
                // 恒成立 所以每次带来的 eviction 和 store B[j][i +
                // 4]只会带来一次MISS，和原来的操作是一样的

                B[j + 4][i] =
                    temp5; // 将原B右上 赋值到 B左下， 这样A右上也就完成了翻转
                B[j + 4][i + 1] = temp6;
                B[j + 4][i + 2] = temp7;
                B[j + 4][i + 3] = temp8;
            }
